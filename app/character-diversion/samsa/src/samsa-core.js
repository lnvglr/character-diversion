...
let CONFIG = {
  ...
  defaultGlyph: [
    ...
    'afii10065',
    'a-cy',
    'zero', // PostScript names: Latin, Russian, Cyrillic (or should we use the cmap? because some fonts lack ps names)
  ],
...
if (CONFIG.isNode) {
  ...
  Buffer.prototype.setUint32 = function (p, v) {
    this.writeUInt32BE(v, p)
  }
  Buffer.prototype.setInt32 = function (p, v) {
    this.writeInt32BE(v, p)
  }
  Buffer.prototype.setUint16 = function (p, v) {
    this.writeUInt16BE(v, p)
  }
  Buffer.prototype.setInt16 = function (p, v) {
    this.writeInt16BE(v, p)
  }
  Buffer.prototype.setUint8 = function (p, v) {
    this.writeUInt8(v, p)
  }
  Buffer.prototype.setInt8 = function (p, v) {
    this.writeInt8(v, p)
  }
  ...
  Buffer.prototype.getTag = function (p, length = 4) {
    ...
    while (p < p_end) {
      ...
      if (ch >= 32 && ch < 126)
        // valid chars in tag data type https://www.microsoft.com/typography/otspec/otff.htm
...
function copyBytes(source, target, zs, zt, n) {
  ...
  for (let i = 0; i < n; i++) tgt[i] = src[i] // this seems to be the quickest way to copy between two ArrayBuffers, see benchmark at https://www.measurethat.net/Benchmarks/Show/7537/0/copy-arraybuffer-dataview-vs-uint8arrayset-vs-float64ar
}
...
function SamsaGlyph(init) {
  ...
  this.tvts = init.tvts || []
  this.curveOrder =
    init.curveOrder || (this.font ? this.font.curveOrder : undefined)
}
...
SamsaGlyph.prototype.decompose = function (tuple, params) {
  ...
  if (this.numContours >= 0) {
    if (!params && !tuple)
      // optimization for case when there’s no offset and no transform (we can make this return "this" itself: the decomposition of a simple glyph is itself)
      return this
  ...
  simpleGlyph.points.push(
    [0, 0, 0],
    [iglyph.points[iglyph.points.length - 3][0], 0, 0],
    [0, 0, 0],
    [0, 0, 0]
  )
...
SamsaGlyph.prototype.instantiate = function (userTuple, instance, extra) {
  ...
  if (extra && extra.roundDeltas === false) round = false

  if (config.visualization) newGlyph.tvtsVisualization = []
  ...
  if (userTuple === null && instance) userTuple = instance.tuple
  ...
  this.tvts.forEach((tvt, t) => {
    ...
    for (let a = 0; a < font.axes.length; a++) {
      ...
      if (peak != 0) {
        ...
        } else if (ua < peak) S *= (ua - start) / (peak - start)
        else if (ua > peak) S *= (end - ua) / (end - peak)
        //else if (ua == peak)
    ...
    if (S != 0) {
      ...
      if (
        ...
        this.numContours > 0 &&
        touched.length > 0 &&
        !config.instantiation.ignoreIUP
      ) {
        // it would be nice to check "touched.length < glyph.points.length" but that won’t work with sparse arrays, and must also think about phantom points
        ...
        for (let c = 0, startPt = 0; c < this.numContours; c++) {
          ...
          for (let p = startPt; p != firstPrecPt; ) {
            ...
            if (touched[p] && !touched[pNext]) {
              // found a precPt
              // get precPt and follPt
              precPt = p
              follPt = pNext
              if (firstPrecPt == -1) firstPrecPt = precPt
              do {
              ...
              for (let xy = 0; xy <= 1; xy++) {
                ...
                for (
                  let q = pNext, D, T, Q;
                  q != follPt;
                  q = ((q - startPt + 1) % numPointsInContour) + startPt
                ) {
                  Q = this.points[q][xy]
                  if (pA == pB) D = dA == dB ? dA : 0
                  else {
                    if (Q <= pA && Q <= pB) D = pA < pB ? dA : dB
                    else if (Q >= pA && Q >= pB) D = pA > pB ? dA : dB
                    else {
              ...
              p = follPt
            } else if (pNext == startPt && firstPrecPt == -1)
              // failed to find a precPt, so abandon this contour
              break
            else p = pNext
          }
  ...
  if (size > 0) {
      ...
      while (pc < tupleNumPoints) {
        ...
        if (pc + runLength <= tupleNumPoints) {
          for (let r = 0; r < runLength; r++) {
            if (pointsAreWords) (pointInc = data.getUint16(ps)), (ps += 2)
            else (pointInc = data.getUint8(ps)), ps++
            pointId += pointInc
    ...
    if (node) {
      ...
      read(
        ...
        0,
        size,
        font.tables['gvar'].offset +
          font.tables['gvar'].data.offsetToData +
          offset
      )
      p = 0
    } else {
    ...
    if (tuplesSharePointNums) {
      ...
      let tupleNumPoints = data.getUint8(ps)
      ps++
      if (tupleNumPoints & 0x80)
        (tupleNumPoints = 0x100 * (tupleNumPoints & 0x7f) + data.getUint8(ps)),
          ps++
      else tupleNumPoints &= 0x7f

      if (tupleNumPoints == 0) {
        // allPoints: would be better to use the allPoints flag in the tuple
    ...
    for (let t = 0; t < tupleCount; t++) {
      ...
      if (tupleIntermediate) {
        // for intermediates, the peak may still be embedded
        for (a = 0; a < font.axisCount; a++) {
      ...
      if (tuplePrivatePointNumbers) {
        ...
        if (tupleNumPoints & 0x80)
          (tupleNumPoints =
            0x100 * (tupleNumPoints & 0x7f) + data.getUint8(ps)),
            ps++
        else tupleNumPoints &= 0x7f

        if (tupleNumPoints == 0) {
          // 0 means "all points", in which case this tuple doesn’t store pointIds
          impliedAllPoints = true
          tvt.iup = false
      ...
      let unpacked = [],
        r
      while (unpacked.length < tupleNumPoints * 2) {
        ...
        switch (bytesPerNum) {
          case 0:
            for (r = 0; r < runLength; r++) unpacked.push(0)
            break
          case 1:
            for (r = 0; r < runLength; r++)
              unpacked.push(data.getInt8(ps)), ps++
            break
          case 2:
            for (r = 0; r < runLength; r++)
              unpacked.push(data.getInt16(ps)), (ps += 2)
            break
        }
      }
...
SamsaGlyph.prototype.recalculateBounds = function () {
  if (this.numContours < 0) {
    ...
    for (let pt = 0; pt < this.numPoints; pt++) {
      ...
      else if (xMax < point[0]) xMax = point[0]
      if (yMin > point[1]) yMin = point[1]
      else if (yMax < point[1]) yMax = point[1]
    }
    this.xMin = xMin
    this.yMin = yMin
    this.xMax = xMax
...
SamsaGlyph.prototype.maxCompiledSize = function () {
  if (this.numContours > 0)
    return (
      ...
      this.numPoints * (2 + 2 + 1)
    )
  else if (this.numContours < 0)
    return (
      5 * 2 + (4 + 4) * 2 * this.components.length + 2 + this.instructionLength
    )
  else return 0
}
...
SamsaGlyph.prototype.compile = function (
  buf,
  startOffset = 0,
  metrics,
  compress = true
  ...
  if (this.numContours > 0) {
    ...
    if (compress) {
      ...
      for (pt = 0; pt < numPoints; pt++) {
        ...
        if (X == 0) f |= 0x10
        else if (X >= -255 && X <= 255) f |= X > 0 ? 0x12 : 0x02

        if (Y == 0) f |= 0x20
        else if (Y >= -255 && Y <= 255) f |= Y > 0 ? 0x24 : 0x04
      ...
      if (font.config.glyf.overlapSimple) flags[0] |= 0x40
      ...
      for (pt = 0; pt < numPoints; pt++) {
        ...
        if (rpt < 2) {
          ...
          if (rpt == 2) buf.setUint8(p - 2, flags[pt] | 0x08) // set repeat bit on the pre-previous flag byte
      ...
      for (pt = 0; pt < numPoints; pt++) {
        ...
        else buf.setInt16(p, dx[pt]), (p += 2)
      }
      for (pt = 0; pt < numPoints; pt++) {
        if (dy[pt] == 0) continue
        if (dy[pt] >= -255 && dy[pt] <= 255)
          buf.setUint8(p, dy[pt] > 0 ? dy[pt] : -dy[pt]), p++
        else buf.setInt16(p, dy[pt]), (p += 2)
      }
    } else {
      // compress != true
      ...
      // overlap signal for Apple
      if (font.config.glyf.overlapSimple) buf.setUint8(p, points[0][2] | 0x40)
  ...
  // 2. COMPOSITE glyph
    ...
    for (let c = 0; c < this.components.length; c++) {
      ...
      if (c < this.components.length - 1) flags |= 0x0020 // MORE_COMPONENTS
      if (component.flags & 0x0200) flags |= 0x0200 // USE_MY_METRICS (copy from the original glyph)
  ...
  switch (this.curveOrder) {
    ...
    case 2:
      ...
      for (endPt of this.endPts) {
        ...
        for (p = startPt; p <= endPt; p++) {
          ...
          contour.push(pt)
          if (pt[2] == 0 && pt_[2] == 0)
            // if we have 2 consecutive off-curve points...
    ...
    case 3:
      ...
      for (endPt of this.endPts) {
        ...
        if (contourLen >= 1 && this.points[startPt][2] == 1) firstOnPt = 0
        ...
        if (firstOnPt !== undefined) {
          ...
          for (p = 0; p < contourLen; p++) {
            ...
            if (p == 0) path += `M${pt[0]} ${pt[1]}`
              ...
              if (pt[2] == 0) {
                // off-curve point (consumes 3 points)
              ...
              if (p == (contourLen + firstOnPt - 1) % contourLen) path += 'Z'
            }
          }
        }
...
SamsaGlyph.prototype.ufo = function () {
  ...
  for (let endPt of this.endPts) {
    ...
    for (let pt = startPt; pt <= endPt; pt++) {
      ...
      let prevPoint =
        this.points[
          startPt + ((ptInContour - 1 + numContourPoints) % numContourPoints)
        ]
      let typeString = ''
      if (point[2] == 1) {
...
function SamsaFont(init, config) {
  ...
  if (this.config.isNode) {
    ...
    if (!init.outFile) this.outFile = this.config.outFileDefault
  }
  ...
  this.parse = () => {
    ...
    switch (font.fingerprint) {
      ...
      default:
        font.errors.push(
          'Invalid first 4 bytes of the file. Must be one of: 0x00010000, 0x74727565, 0x4f54544f'
        )
        break
    }
    if (!font.errors.length) {
      ...
      if (font.numTables > config.sfnt.maxNumTables)
        font.errors.push(
          `numTables (${font.numTables}) exceeds config.sfnt.maxNumTables (${config.sfnt.maxNumTables})`
        )
      else {
        /////////////////////////////////////////////////////////////////////////////////
    ...
    ;[
      ...
      if (font.tables[tag]) font.parseSmallTable(tag)
    })
  ...
  this.parseSmallTable = (tag) => {
    ...
    if (node) {
      ...
      data = new DataView(
        font.data.buffer,
        font.tables[tag].offset,
        font.tables[tag].length
      )
    }
    ...
    switch (tag) {
      ...
      case 'hmtx':
        ...
        for (let m = 0; m < font.numGlyphs; m++) {
          if (m < font.tables['hhea'].data.numberOfHMetrics) {
            ...
            p += 2 // skip lsb
          } else font.widths[m] = font.widths[m - 1]
        }
        break // hmtx end
      ...
      case 'name':
        ...
        for (let n = 0; n < table.count; n++) {
          ...
          if (nameRecordEnd > font.tables['name'].length)
            // safety check
            continue
      ...
      case 'post':
        ...
        if (table.format == 0x00020000) {
          ...
          for (let g = 0; g < font.numGlyphs; g++) {
            ...
            font.glyphNames[g] =
              gni < 258 ? config.postscriptNames[gni] : extraNames[gni - 258]
          }
        }
        break // post end
      ...
      case 'loca':
        ...
        for (let g = 1; g <= font.numGlyphs; g++) {
          ...
          if (indexToLocFormat) font.glyphOffsets[g] = data.getUint32(4 * g)
          else font.glyphOffsets[g] = 2 * data.getUint16(2 * g)

          font.glyphSizes[g - 1] =
            font.glyphOffsets[g] - font.glyphOffsets[g - 1]
        }
        break // loca end
      ...
      case 'fvar':
        ...
        for (let a = 0; a < table.axisCount; a++) {
          ...
          if (axis.flags & 0x0001) axis.hidden = true
          ;(axis.axisNameID = data.getUint16(p)), (p += 2)
          axis.name = font.names[axis.axisNameID]
          if (axis.name === undefined) axis.name = axis.tag // for fonts without name table, e.g. optimized webfonts
        ...
        for (let i = 0; i < table.instanceCount + 1; i++) {
          // +1 allows for default instance

          let instance

          if (i == 0) instance = font.instances[0]
          else {
            instance = {
          ...
          font.axes.forEach((axis, a) => {
            ...
            if (i == 0) instance.tuple[a] = axis.default // user-facing value
            else (instance.tuple[a] = data.getInt32(p) / 65536), (p += 4) // user-facing value
      ...
      case 'avar':
        ...
        if (!(table.majorVersion === 1 && table.minorVersion === 0)) break
        p += 2
        ;(table.axisCount = data.getUint16(p)), (p += 2)
        for (let a = 0; a < table.axisCount; a++) {
          // not font.axisCount, because we have not yet parsed fvar
          font.avar[a] = []
          let positionMapCount = data.getUint16(p)
          p += 2 // TODO: if positionMapCount==3 (NOOP), don’t store anything
          for (let m = 0; m < positionMapCount; m++) {
            ;(font.avar[a][m] = [data.getF2DOT14(p), data.getF2DOT14(p + 2)]),
              (p += 4) // = [<fromCoordinate>,<toCoordinate>]
          }
        }
        break // avar end
      ...
      case 'gvar':
        ...
        for (let g = 0; g < font.numGlyphs; g++) {
          if (table.flags & 0x01)
            // offsets are Offset32
            font.tupleOffsets[g + 1] = data.getUint32(20 + 4 * (g + 1))
          // offsets are 2*Offset16
          else font.tupleOffsets[g + 1] = 2 * data.getUint16(20 + 2 * (g + 1))
        ...
        if (font.tables['GSUB']) {
          ...
          if (table.majorVersion == 1 && table.minorVersion <= 1) {
            ...
            for (let f = 0; f < table.featureCount && f < 100; f++, p += 6) {
              ...
              if (feature.tag !== false) {
                ...
                if (feature.featureParams) {
                  p_ =
                    table.featureListOffset +
                    feature.offset +
                    feature.featureParams
                  ;(feature.featureParamsVersion = data.getUint16(p_)),
                    (p_ += 2)
            ...
            for (let lu = 0; lu < table.lookupCount; lu++) {
              ...
              for (let st = 0; st < lookup.subTableCount; st++) {
                p =
                  table.lookupListOffset +
                  lookup.offset +
                  lookup.subtableOffsets[st]
                  ...
                  case 1:
                    ...
                    if (lookup.substFormat == 1) {
                      ...
                      for (let g = 0; g < lookup.glyphCount; g++)
                        (lookup.substituteGlyphIds[g] = data.getUint16(p)),
                          (p += 2)
                    }
                    ...
                    if (format == 1) {
                      let glyphCount = data.getUint16(p)
                      p += 2
                      ...
                      for (let r = 0; r < rangeCount; r++) {
                        let start = data.getUint16(p),
                          end = data.getUint16(p + 2)
            ...
            if (table.featureVariationsOffset) {
              ...
              if (data.getUint16(p) == 1 && data.getUint16(p + 2) == 0) {
                // check version
                  ...
                  if (data.getUint16(p) == 1 && data.getUint16(p + 2) == 0) {
                    ...
                    for (let su = 0; su < subsCount; su++) {
                      ...
                      let p_ =
                        table.featureVariationsOffset +
                        ftsOffset +
                        offsetToFeatureTable // alternateFeatureTable offset
                      ...
                      featureVariation.substitutions.push([
                        featureIndex,
                        lookups,
                      ])
                    }
          ...
          } else font.errors.push('GSUB: Unknown version')
        }
      ...
      case 'cmap':
        ...
        for (let t = 0; t < table.numTables; t++) {
          ...
          if (
            encodingRecord.platformID == 3 &&
            encodingRecord.encodingID == 1
          ) {
  ...
  this.parseGlyph = (g) => {
    ...
    // printing glyph
      ...
      if (glyph.numContours > 0) {
        ...
        for (pt = 0; pt < glyph.numPoints; ) {
          ...
          if (flag & 0x08) {
            ...
            for (r = 0; r < repeat; r++) flags[pt++] = flag
          }
        }
        ...
        if (flags.length == glyph.numPoints) {
          flags.forEach((flag, pt) => {
            switch (
              ...
              case 0x00:
                x = x_ + data.getInt16(p)
                p += 2
                break
              case 0x02:
                x = x_ - data.getUint8(p)
                p++
                break
              case 0x10:
                x = x_
          ...
          flags.forEach((flag, pt) => {
            switch (
              ...
              case 0x00:
                ;(y = y_ + data.getInt16(p)), (p += 2)
                break
              case 0x04:
                ;(y = y_ - data.getUint8(p)), p++
                break
              case 0x20:
                y = y_
                break
      ...
      // - when we process these glyphs, we look at glyph.numContours and glyph.points, but NOT glyph.numPoints
        ...
        do {
          ...
          if (flag & 0x0002) {
            ...
            if (flag & 0x0001) {
              ...
              ;(component.offset = [data.getInt16(p), data.getInt16(p + 2)]),
                (p += 4)
            } else {
              ;(component.offset = [data.getInt8(p), data.getInt8(p + 1)]),
                (p += 2)
            }
        ...
        // error
        font.errors.push(
          'glyf: Glyph ' + g + ' has 0 contours, but non-zero size'
        )
      }
    }
  ...
  this.exportInstance = (instance) => {
    ...
    // set up timer
    let timerStart
    if (typeof performance !== 'undefined') timerStart = performance.now()
    else timerStart = 0
    ...
    if (!instance.filename) instance.filename = font.outFile
    ...
    } else fontBuffer = new DataView(new ArrayBuffer(font.config.sfnt.maxSize))
    ...
    font.tableDirectory.forEach((table) => {
      if (!font.config.instantiation.skipTables.includes(table.tag))
        // if this table is not in the "skipTables" list
    ...
    if (font.addTables) {
      font.addTables.forEach((table) => {
        ...
        if (table.tag == 'CFF2' || table.tag == 'CFF ') {
          // if we add a CFF/CFF2 table, we no longer want glyf or loca
          if (newTables['glyf']) delete newTables['glyf']
          if (newTables['loca']) delete newTables['loca']
        }
      })
    }
    ...
    let locaBuf, hmtxBuf
    let aws = [],
      lsbs = [] // metrics, calculated when we instantiate each glyph
    ...
    newTableDirectory.forEach((table, t) => {
      ...
      switch (table.tag) {
        case 'glyf':
          ...
          p = 0
          glyfBuffer = node
            ? flushGlyfBuffer(glyfBuffer)
            : new DataView(fontBuffer.buffer, position)
          ...
          for (let g = 0; g < font.numGlyphs; g++) {
            ...
            if (
              iglyph.numContours &&
              node &&
              p + iglyph.maxCompiledSize() + glyfBufSafetyMargin >
                font.config.glyf.bufferSize
            )
            ...
            // padding
            if (p % 2) glyfBuffer.setUint8(p, 0), p++
            ...
            if (node) newLocas[g + 1] = glyfBufferOffset + p
            else newLocas[g + 1] = p
          ...
          if (node) glyfBuffer = flushGlyfBuffer(glyfBuffer)
        ...
        case 'name':
          ...
          const nameBuf = new DataView(
            new ArrayBuffer(font.config.name.maxSize)
          )
          let newNames = []
          ...
          font.names.forEach((name, nameID) => {
            if (instance.type != 'default') {
              switch (nameID) {
                ...
                case 17:
                  name = subfamilyName
                  if (!font.names[17]) newNames.push([17, name]) // add typographic subfamily name if it’s not there
                  break
          ...
          newNames.forEach((kv, n) => {
            let nameID = kv[0],
              name = kv[1]
            ;[3, 1, 0x0409, nameID, 2 * name.length, p - offsetStr].forEach(
              (nri, i) => {
            ...
            for (
              let c = 0;
              c < name.length;
              c++ // write each string (2 bytes per character)
            )
          ...
          // 3. write name data
          table.length = p
          if (node) write(fdw, nameBuf, 0, table.length, table.offset)
          else copyBytes(nameBuf, fontBuffer, 0, table.offset, table.length)
          position += table.length
          break
        ...
        default:
          ...
          if (node) {
            ...
            if (originalTable.data && originalTable.data.buffer) {
              newTableBuffer = new DataView(
                originalTable.data.buffer.buffer.slice(
                  originalTable.offset,
                  originalTable.offset + originalTable.length
                )
              ) // slice() copies an arrayBuffer
            } else {
              newTableBuffer = new DataView(
                fontBuffer.buffer,
                table.offset,
          ...
          if (table.tag == 'head') {
            let longDateTime =
              ...
              ((1970 - 1904) * 365 + Math.floor((1970 - 1904) / 4) + 1) *
                24 *
                60 *
                60 // seconds since 1904-01-01 00:00:00... new Date().getTime() is UTC; Math.floor((1970-1904)/4)+1 = 17 leap years between 1904 and 1970
          ...
          if (node) write(fdw, newTableBuffer, 0, table.length, table.offset)
          else
      ...
      if (padLength) {
        if (node) write(fdw, zeroBuffer, 0, padLength, position) // write at current position
    ...
    for (let g = 0; g < font.numGlyphs; g++) {
      if (aws[g] > 0)
        // negative values (not in hmtx spec, but gvar processing might cause them) should have been fixed already
        hmtxBuf.setUint16(4 * g, aws[g])
      if (lsbs[g]) hmtxBuf.setInt16(4 * g + 2, lsbs[g])
    }
    if (node)
      write(fdw, hmtxBuf, 0, 4 * font.numGlyphs, newTables['hmtx'].offset)

    // [4c] fix the font header
    fileHeaderBuf.setUint32(
      0,
      newTables['CFF2'] ? 0x4f54544f : font.fingerprint
    ) // "OTTO" if CFF2
    ...
    if (node) write(fdw, fileHeaderBuf, 0, fileHeaderSize, 0)
    ...
    else instance.timer = 0

    instance.size = position

    // [6] close file or return binary
    if (node) font.config.fs.closeSync(fdw)
    else return new Uint8Array(fontBuffer.buffer, 0, position)
  }
  ...
  this.getNamedInstances = function () {
    ...
    this.instances.forEach((instance) => {
      if (instance.type == 'named') instances.push(instance)
    })
    return instances
  }
  ...
  this.addInstance = (fvs, options) => {
    ...
    if (fvs) {
      this.axes.forEach((axis, a) => {
        instance.fvs[axis.tag] =
          fvs[axis.tag] === undefined ? axis.default : 1.0 * fvs[axis.tag]
  ...
  this.fvsToTuple = (fvs) => {
    ...
    this.axes.forEach((axis, a) => {
      let val =
        !fvs || fvs[axis.tag] === undefined ? axis.default : 1.0 * fvs[axis.tag]
  ...
  this.tupleToFvs = (tuple) => {
    ...
    this.axes.forEach((axis, a) => {
        ...
        for (let m = 0; m < map.length; m++) {
          if (map[m][1] >= n) {
            if (map[m][1] == n) {
              ...
              if (map[m][1] == map[m - 1][1]) n = map[m - 1][0]
              else
                n =
                  map[m - 1][0] +
                  (map[m][0] - map[m - 1][0]) *
                    ((n - map[m - 1][1]) / (map[m][1] - map[m - 1][1]))
            }
            break
          }
        }
      ...
      if (n > 0) fvs[axis.tag] += (axis.max - axis.default) * n
      else if (tuple[a] < 0) fvs[axis.tag] += (axis.default - axis.min) * n
    })
    return fvs
  }
  ...
  this.axisIndices = (tag) => {
    ...
    let indices = []
    this.axes.forEach((axis, a) => {
      if (axis.tag == tag) indices.push(a)
    })
    return indices
  }
  ...
  this.axisNormalize = (axis, t, avarIgnore) => {
      ...
      if (t > axis.max) n = 1
        ...
        n =
          axis.max == axis.default
            ? 0
            : (t - axis.default) / (axis.max - axis.default)
    } else {
      if (t < axis.min) n = -1
      else
        n =
          axis.min == axis.default
            ? 0
            : (axis.default - t) / (axis.min - axis.default)
    }
    ...
    if (this.avar && this.avar[axis.id] && !avarIgnore) {
      ...
      for (let m = 0; m < map.length; m++) {
        if (map[m][0] >= n) {
          if (map[m][0] == n) n = map[m][1]
          else
            n =
              map[m - 1][1] +
              (map[m][1] - map[m - 1][1]) *
                ((n - map[m - 1][0]) / (map[m][0] - map[m - 1][0]))
  ...
  this.defaultGlyphId = () => {
    ...
    for (let d = 0; d < this.config.defaultGlyph.length; d++) {
      let g,
        name = this.config.defaultGlyph[d]
    ...
    for (let g = 0; g < this.numGlyphs; g++) {
      if (
        this.glyphs[g].numContours > 0 &&
        g > 0 &&
        this.glyphNames[g] != '.notdef'
      ) {
        return g // inelegant but concise
      }
    }
  ...
  this.featureVariationsGlyphId = (g, tuple) => {
    ...
    if (!font.featureVariations) return newGlyphId
    ...
    for (let featureVariation of font.featureVariations) {
      ...
      for (let co = 0; co < conds.length && match; co++) {
        ...
        if (tuple[cond[0]] < cond[1] || tuple[cond[0]] > cond[2]) match = false
      }
          ...
          for (let lu = 0; lu < lookups.length; lu++) {
            ...
            if (gIndex !== -1) {
              if (lookup.substFormat == 1)
                // deltaGlyphId method
                newGlyphId =
                  (g + lookup.deltaGlyphId + 0x10000) %
                  0x10000 // we + and % 65536 because (g+lookup.deltaGlyphId) can be negative
              else if (lookup.substFormat == 2)
                // substituteGlyphIds method
  ...
  this.featureVariationsBoxes = (g) => {
    ...
    if (!font.featureVariations) return boxes
