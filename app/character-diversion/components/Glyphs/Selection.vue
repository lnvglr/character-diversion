<template>
	<div v-if="$state.opinion.font && $state.opinion.form.attributes" class="grid gap-1 p-1"
		:class="`grid-cols-autofill-${gridSize}`">
		<Card :hoverable="$state.opinion.formActive" class="flex justify-center w-full relative overflow-hidden" :class="{
			'opacity-10': dim(glyph.id),
			'cursor-pointer': $state.opinion.formActive,
			[`h-${gridSize}`]: true
		}" v-for="(glyph, k) in filteredGlyphs" :key="glyph.id"
          ...
          @pointerenter="active && (last = k)" -->
			<GlyphsMiniGlyph :class="`text-${fontSize}`" :glyph="glyph" :tuple="$state.opinion.form.attributes.axes"
				:annotations="annotations" :frame="frame" :edit="edit" :intersection="intersection" :watcher="[gridSize]"
				:title="glyph.name" />
			<Input v-if="$state.opinion.formActive" type="checkbox" v-model="$state.opinion.selectedGlyphs" :value="glyph.id"
				containerClass="absolute w-fit right-0 p-1" class="info z-10" @click.prevent />
...
export default {
	...
	computed: {
		filteredGlyphs() {
			...
			const references = !q || typeof q !== 'string' || q === "" ? false :
				q.split(',').join(' ').split(' ')
					.map((e: string) => '/' + e)
					.join('')
			...
			return this.$state.opinion.font.glyphs.filter(
				(glyph: SamsaGlyph) => {
					if (
						...
						(!references || references && matchedGlyphs.includes(glyph.id))
					) {
						return glyph
					}
				}
