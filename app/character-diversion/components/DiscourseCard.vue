<template>
  <Card
    class="min-h-[200px] grid duration-75 hover:rounded-md hover:shadow-[inset_0_0_0_2px_var(--color-beige-300)] dark:hover:shadow-[inset_0_0_0_2px_var(--color-neutral-900)]"
    v-if="discourse"
  >
    <NuxtLink
    ...
    >
      <div class="flex items-center gap-3 mb-5">
        <div
          class="text-5xl text-info-500 bg-beige-100 flex items-center justify-center p-2 rounded-md w-20 h-20 min-w-[5rem] overflow-hidden"
          v-html="fontPreview"
        ></div>
      ...
      <div class="mt-auto py-2">
        <span class="text-xs flex gap-2">
          <span class="flex gap-1"
            >{{ $t("opinion", opinions.length)
            }}<UITag :round="true">{{ opinions.length }}</UITag></span
          >
          <span class="flex gap-1"
            >{{ $t("participant", participants.length)
            }}<UITag :round="true">{{ participants.length }}</UITag></span
          >
        </span>
      </div>
      <div
        class="flex justify-between items-center border-t border-beige-200 dark:border-neutral-600 pt-4"
      >
        <Author :post="discourse" class="text-sm" />
        <Button
          v-if="
            $strapi.user?.id && discourse.attributes.author?.data?.id === $strapi.user?.id
          "
...
export default defineComponent({
  ...
  computed: {
    ...
    participants() {
      return [
        ...new Set(
          this.discourse?.attributes.opinions?.data.map(
            (opinion: Opinion) => opinion.attributes.author?.data?.id
          )
        ),
      ];
    },
