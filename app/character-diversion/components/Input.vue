<template>
  <div class="input-container" :class="containerClass">
    ...
    <component
      ...
      v-bind="$attrs"
      :empty="empty || null"
      class="border-beige-200 w-full focus:outline-info-500"
    ...
    />
    <div
      v-if="maxlength || submitOnEnter || $attrs.onCancel"
    ...
    <label class="placeholder absolute" v-if="label && type !== 'range'">{{
      label
    }}</label>
  </div>
</template>
...
export default defineComponent({
  ...
  computed: {
    empty() {
      return (
        (!this.modelValue || String(this.modelValue) === "") && 0 !== this.modelValue
      );
    },
    border() {
      switch (this.type) {
  ...
  methods: {
    updateValue(val: number | string | null) {
      ...
      switch (type) {
        case "number":
          ...
          break;
        case "string":
          value = String(val).length !== 0 ? val : null;
          break;
      }
...
:is(input, textarea):not([type="range"]):not(:disabled):where(:focus, :active, :not([empty]))
  ~ .placeholder {
